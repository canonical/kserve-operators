...
LLMISVC_EXAMPLE_PATH = TEST_DATA_DIR / "llmisvc_test_llm_scheduler_small.yaml"
# LLMInferenceService examples to deploy and run predictions against, as
# (custom-resource name, manifest path) pairs. Each example is applied,
# verified, predicted against, and then deleted before the next one so the
# cluster's resource usage stays bounded (important on constrained CI runners).
LLMISVC_EXAMPLES = [
    ("test-llm-scheduler-small", LLMISVC_EXAMPLE_PATH),
    ("test-llm-prefill-decode", TEST_DATA_DIR / "llmisvc_test_llm_prefill_decode.yaml"),
]
...
def test_run_example(juju: jubilant.Juju, example_name: str, example_path: Path):
    ...
    apply_llmisvc_example(manifest_path=str(example_path), name=example_name)
    ...
    # Delete this example before the next one runs to keep cluster resource
    # usage bounded on constrained CI runners.
    logger.info("Example '%s': deleting after validation", example_name)
    delete_llmisvc_example(manifest_path=str(example_path), name=example_name)
...
def test_remove_charms_leaves_no_charm_resources(juju: jubilant.Juju):
    ...
    # The kserve-llmisvc charm deletes any remaining LLMInferenceService custom
