...
async def test_deploy_resource_dispatcher(ops_test: OpsTest):
    ...
    await ops_test.juju(
        ...
        "--channel",
        RESOURCE_DISPATCHER.channel,
        "--revision",
        str(RESOURCE_DISPATCHER_REVISION),
        "--trust",
        check=True,
    )
    await ops_test.model.wait_for_idle(
...
async def test_user_namespace_has_new_manifests(
    ops_test: OpsTest, lightkube_client: lightkube.Client, test_namespace: str
):
    """Create a user namespace and check the dispatched S3 Secret and ServiceAccount.

    After swapping to s3-integrator, the Secret rendered into user namespaces should be
    built from the data provided over the `s3-credentials` relation, not the prior MinIO
