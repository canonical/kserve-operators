...
class KServeControllerCharm(CharmBase):
    ...
    def reconcile_authorization_policies(self):
        ...
        TODO: rephrase
        Only relevant for RawDeployment. In Serverless mode, this function does nothing.
        Create an AuthorizationPolicy to allow any workload to talk to the controller.
        Needed for communication to the kserve-webhook-server service.
        """
        ap_raw = AuthorizationPolicy(
            metadata=ObjectMeta(
                name=f"{self.app.name}-allow-all",
                namespace=self.model.name,
            ),
